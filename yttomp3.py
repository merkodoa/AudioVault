...
def proces_pobierania(url, sciezka_zapisu):
    ydl_opts = {
...
def klikniecie_pobierz():
    ...
    watek = threading.Thread(target=proces_pobierania, args=(link, folder))
    watek.start()
...
przycisk_pobierz = ctk.CTkButton(app, text="Download mp3", height=40, font=("Arial", 14, "bold"), command=klikniecie_pobierz)
